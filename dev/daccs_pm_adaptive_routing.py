"""Compare adaptive temporal routing cutoffs for the DACCS case study.

The script loads the same data package and foreground inventories as the DACCS
depth-sweep notebook/script, runs ``Trails.temporal_routing`` in adaptive mode,
and writes tabular diagnostics plus optional Sankey and temporal-score plots.
It is intended for method-development diagnostics, not for package runtime.

The activity-year Sankey visualizes the routed graph with node and link sizes
based on static LCIA score potential. A representative path to the deepest
routed depth is always retained so low-score tails are visible even when most
links are filtered for readability.
"""
...
DEPTH_SWEEP_PATH = REPO_ROOT / "dev" / "daccs_pm_depth_sweep.py"
...
depth_runner = _load_depth_runner()
runner = depth_runner.runner
helpers = depth_runner.helpers
...
def run(args: argparse.Namespace) -> int:
    ...
    if static_score is None:
        raise RuntimeError(
            "No static row found in depth-sweep CSV; run daccs_pm_depth_sweep.py first."
        )
