...
class Trails:
    ...
    def reset_inventory(self, *, attribute_to_roots: bool = False) -> None:
        ...
        # ------------------------------------------------------------------
        # Chunked builder (single source of truth)
        # Each append stores: act, year_idx, (optional root), flows[], values[]
        # finalize_inventory expands scalar coords using np.repeat.
        # ------------------------------------------------------------------
        self._inv_chunk_act: list[int] = []
        self._inv_chunk_year: list[int] = []
        self._inv_chunk_root: list[int] = []  # only used if _inventory_has_root
    ...
    def _get_B_row_max_for_t_act(self, t: int, act: int) -> float:
        """Return cached max absolute value for a given (t, act) row."""
        if not hasattr(self, "_B_row_max_cache"):
            self._B_row_max_cache = {}  # type: ignore[attr-defined]
        cache = self._B_row_max_cache  # type: ignore[attr-defined]
        key = (int(t), int(act))
        cached = cache.get(key)
        if cached is not None:
            return float(cached)

        row_ptr, _, data_sorted = self._get_B_row_cache_for_t(int(t))
        start = int(row_ptr[int(act)])
        end = int(row_ptr[int(act) + 1])
        if end <= start:
            cache[key] = 0.0
            return 0.0
        max_val = float(np.max(np.abs(data_sorted[start:end])))
        cache[key] = max_val
        return max_val
    ...
    def _accumulate_temporalized_biosphere_inventory_core(
        ...
        *,
        ...
        get_B_row_cache_for_t = self._get_B_row_cache_for_t
        get_B_row_index_map_for_t_act = self._get_B_row_index_map_for_t_act
        get_B_row_max_for_t_act = self._get_B_row_max_for_t_act
        get_numba_ported_kernel = self._get_numba_ported_kernel
        get_numba_matrix_kernel = self._get_numba_matrix_kernel
        append_bulk = self._append_inventory_entries_bulk
        append_chunk = self._append_inventory_entries
        row_ptr = ctx.row_ptr
        ...
        for act_idx, supply_amt in supply_by_activity.items():
            ...
            if min_amt:
                row_max = get_B_row_max_for_t_act(ctx.t, a)
                if abs(supply_amt) * row_max < min_amt:
                    continue
            ...
            if td_struct is None:
                ...
                max_weight = 1.0
                if ported_weights_arr is not None and ported_weights_arr.size:
                    max_weight = max(max_weight, float(np.max(np.abs(ported_weights_arr))))
                max_abs_vals = float(np.max(np.abs(vals_full)))
                td_struct = (
                    no_td_idx,
                    ported_flow_idx_arr,
                    ported_offsets_arr,
            ...
            if no_td_idx is not None:
                ...
                if idx.size:
                    flows_no_td = flows_full[idx]
                    values_no_td = scaled_full[idx]
                else:
                    flows_no_td = None
                    values_no_td = None
            else:
            ...
            if min_amt:
                if abs(supply_amt) * max_abs_vals * max_weight < min_amt:
                    continue

            td_flows_list: list[np.ndarray] = []
            td_years_list: list[np.ndarray] = []
            td_values_list: list[np.ndarray] = []

            if flows_no_td is not None and values_no_td is not None:
                years_no_td = np.full(flows_no_td.size, base_year, dtype=np.int64)
                td_flows_list.append(flows_no_td.astype(np.int64, copy=False))
                td_years_list.append(years_no_td)
                td_values_list.append(values_no_td.astype(np.float64, copy=False))
            ...
            if ported_flow_idx is not None and ported_offsets is not None:
                ...
                if idx.size:
                    kernel = get_numba_ported_kernel()
                    ...
                    if min_amt:
                        ...
                        if not m.any():
                            flows_use = np.array([], dtype=flows_use.dtype)
                            years_use = np.array([], dtype=years_use.dtype)
                    ...
                    if flows_use.size:
                        td_flows_list.append(flows_use.astype(np.int64, copy=False))
                        td_years_list.append(years_use.astype(np.int64, copy=False))
                        td_values_list.append(contrib.astype(np.float64, copy=False))
            ...
            if matrix_entries:
                if isinstance(matrix_entries, list):
                    ...
                    for k, (pos_list, tex0) in grouped.items():
                        ...
                        if weights_arr.size:
                            max_mat_weight = max(
                                max_mat_weight, float(np.max(np.abs(weights_arr)))
                            )
                        matrix_groups[k] = (idx_full, offsets_arr, weights_arr)
                    max_weight = max(max_weight, max_mat_weight)
                    td_struct = (
                        no_td_idx,
                        ported_flow_idx,
                        ported_offsets,
                ...
                for k, (idx_full, offsets_arr, weights_arr) in matrix_entries.items():  # type: ignore[union-attr]
                    ...
                    for y_eff, year_weights in year_groups.items():
                        ...
                        if cached_eff is None:
                            cached_eff = get_B_row_cache_for_t(t_eff_i)
                        ...
                        if f_arr.size == 0:
                            continue
                        row_index_map = get_B_row_index_map_for_t_act(t_eff_i, a)
                        matrix_kernel = get_numba_matrix_kernel()
                        ...
                        td_flows_list.append(flows_rep.astype(np.int64, copy=False))
                        td_years_list.append(years_rep.astype(np.int64, copy=False))
                        td_values_list.append(contrib.astype(np.float64, copy=False))

            if td_flows_list:
                flows_all = np.concatenate(td_flows_list)
                years_all = np.concatenate(td_years_list)
                values_all = np.concatenate(td_values_list)
                acts_use = np.full_like(flows_all, inventory_act, dtype=np.int64)
                append_bulk(
                    acts_use,
                    years_all,
                    flows_all,
