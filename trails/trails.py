        ...
        *,
        ...
        for act_idx, supply_amt in supply_by_activity.items():
            ...
            if min_amt:
                ...
                max_weight = 1.0
                if ported_weights is not None and ported_weights.size:
                    max_weight = max(max_weight, float(np.max(np.abs(ported_weights))))
                if matrix_entries:
                    if isinstance(matrix_entries, dict):
                        max_mat_weight = max(
                            (
                                float(np.max(np.abs(w)))
                                for _, (_, _, w) in matrix_entries.items()
                                if w.size
                            ),
                            default=0.0,
                        )
